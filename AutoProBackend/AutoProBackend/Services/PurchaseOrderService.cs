...
public class PurchaseOrderService : IPurchaseOrderService
{
    ...
    public async Task<List<PurchaseOrderResponse>> GetAllAsync(string? status)
    {
        ...
        return await query.OrderByDescending(po => po.Date).Select(po => MapToResponse(po)).ToListAsync();
    }
    ...
    public async Task<(PurchaseOrderResponse? response, bool vendorNotFound, bool partsNotFound)> CreateAsync(CreatePurchaseOrderRequest req)
    {
        ...
        return (MapToResponse(order), false, false);
    }

    public async Task<bool> UpdateStatusAsync(int id, UpdatePurchaseOrderStatusRequest req)
    {
        var order = await _db.PurchaseOrders
            .Include(po => po.Items).ThenInclude(i => i.Part)
            .Include(po => po.Vendor)
