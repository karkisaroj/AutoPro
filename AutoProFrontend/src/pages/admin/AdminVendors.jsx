...
const EMPTY = { name: '', contact: '', phone: '', email: '', address: '', category: 'Multi-Parts', paymentTerms: '' };
...
export default function AdminVendors() {
  ...
  const openAdd  = () => { setEditing(null); setForm(EMPTY); setSaveError(''); setModal(true); };
  const openEdit = (v) => {
    setEditing(v.id);
    setForm({ name: v.name, contact: v.contact, phone: v.phone, email: v.email, address: v.address, category: v.category, paymentTerms: v.paymentTerms || '' });
    setSaveError('');
    setModal(true);
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      setDeleteError(err?.message || 'Failed to delete vendor. Please try again.');
      setConfirm(null);
    }
  };
  ...
  const handleToggle = async (id) => {
    ...
    setVendors(prev => prev.map(v => v.id === id ? { ...v, status: u.isActive ? 'Active' : 'Inactive' } : v));
  };
  ...
  const stats = [
    ...
    { label: 'Active Vendors', value: vendors.filter(v => v.status === 'Active').length.toString(),       color: 'emerald', icon: Store       },
  ...
  return (
    <div className="space-y-6 page-enter">
      ...
      {deleteError && (
        <div className="rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 px-4 py-3 text-sm text-red-700 dark:text-red-300 flex items-center justify-between">
          <span>{deleteError}</span>
          <button onClick={() => setDeleteError('')} className="text-red-400 hover:text-red-600 ml-4 text-lg leading-none">&times;</button>
        </div>
      )}
        ...
        <Modal title={editing ? 'Edit Vendor' : 'Add New Vendor'} onClose={() => setModal(false)} icon={Store}>
          <div className="space-y-4">
            ...
            {[
              ...
              { label: 'Payment Terms',  key: 'paymentTerms', ph: 'Net 30',        col: 'col-span-2' },
